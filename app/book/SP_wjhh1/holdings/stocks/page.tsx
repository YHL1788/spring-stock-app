    ...
    <div className="space-y-8 pb-10 max-w-[1500px] mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                ...
                <div className="flex items-center gap-2">
                    <select
                        value={selectedHoldingAccount}
                        onChange={(e) => setSelectedHoldingAccount(e.target.value)}
                        className="rounded border border-rose-200 bg-white px-2.5 py-1.5 text-xs font-bold text-rose-700 shadow-sm outline-none transition-colors hover:bg-rose-50 focus:ring-2 focus:ring-rose-200"
                        title="按账户重算盈亏分析明细与图表"
                    >
                        <option value="">全部账户</option>
                        {holdingAccountOptions.map(account => (
                            <option key={account} value={account}>{account}</option>
                    ...
                    {selectedHoldingAccount && (
                        <button
                            onClick={() => setSelectedHoldingAccount('')}
                            className="rounded border border-gray-200 bg-white px-2.5 py-1.5 text-xs font-bold text-gray-500 shadow-sm transition-colors hover:bg-gray-50 hover:text-gray-700"
                        >
                            清空账户
                        </button>
                    )}
            ...
            <div className={getTableContainerClass('pnl', 'grid grid-cols-1 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x divide-gray-200')}>
                {renderExpandedTableHeader('pnl', '盈亏分析明细与图表', `账户：${selectedHoldingAccount || '全部账户'}；当前显示 ${chartData.length} 只标的，图表模式与原页面同步。`)}
