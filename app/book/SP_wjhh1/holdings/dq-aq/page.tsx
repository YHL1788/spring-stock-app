...
import { 
  RefreshCw, Database, FileJson, Trash2, X, Save, Loader2, AlertCircle, TrendingUp, LineChart 
} from 'lucide-react';
import { collection, getDocs, doc, updateDoc, addDoc, deleteDoc, serverTimestamp, query, where } from 'firebase/firestore';
    ...
    return (
        <th className={`px-3 py-2 whitespace-nowrap align-top group ${textClass}`}>
            ...
            {filterKey && onFilter && (
                <div className="mt-1">
                    <input 
                        type="text" placeholder="篩選..." value={currentFilter?.[filterKey] || ''}
    ...
    return (
        <div className="bg-white p-4 rounded border border-gray-200 shadow-sm w-full flex flex-col items-center">
            ...
            <div className="flex justify-center gap-4 mt-4 text-[10px] text-gray-500">
                <div className="flex items-center"><span className="w-3 h-0.5 bg-gray-800 mr-1"></span>歷史</div>
                <div className="flex items-center"><span className="w-3 h-0.5 bg-gradient-to-r from-red-400 to-blue-400 mr-1"></span>模擬</div>
...
export default function DQAQHoldingPage() {
    ...
    const fetchMergedRecords = async (lifeCycle: 'living' | 'died'): Promise<MergedRecord[]> => {
        try {
            ...
            const merged = inputs.map(inp => {
                const out = outputs.find(o => o.tradeId === inp.tradeId);
                if (!out) return null; 
                return { tradeId: inp.tradeId, inputId: inp.id, outputId: out.id, inputData: inp, outputData: out, createdAt: inp.createdAt };
    ...
    const fetchDbRecords = async (colName: string) => {
        ...
        try {
            ...
            // 排序：優先看 updatedAt，如果沒有才看 createdAt
            recs.sort((a,b) => {
    ...
    const handleRefreshLiving = async () => {
        ...
        try {
            ...
            for (const record of currentLiving) {
                ...
                if (res.status_msg.includes('存續中') || res.status_msg.includes('訂約但未開始') || res.status_msg.includes('存续中') || res.status_msg.includes('订约但未开始')) {
                    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'sip_trade_dqaq_input_living', record.inputId), cleanInput);
                    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'sip_holding_dqaq_output_living', record.outputId), cleanOutput);
                } else {
                    await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'sip_trade_dqaq_input_died'), cleanInput);
                    await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'sip_holding_dqaq_output_died'), cleanOutput);
            ...
            if (allNewDeliveries.length > 0) {
                ...
                alert(`刷新完畢！有 ${movedToDiedCount} 筆已失效移入歷史庫。本次刷新未產生新的結算/接貨記錄。`);
            }
        } catch(e:any) { alert("刷新失敗: " + e.message); } finally { setLoadingLiving(false); }
    };
    ...
    const handleConfirmDeliveries = async () => {
        ...
        try {
            ...
            for (const rec of pendingDeliveries) {
                ...
                await addDoc(getStockRef, {...clean, createdAt: serverTimestamp()});
            }
            alert("交收流水已成功精準覆寫至 get-stock 接貨庫！");
            setShowDeliveryModal(false);
            setPendingDeliveries([]);
            if(activeDbTab==='sip_holding_dqaq_output_get-stock') fetchDbRecords(activeDbTab);
        } catch(e:any) { alert("覆寫失敗: " + e.message); } finally { setSyncingDeliveries(false); }
    };
    ...
    const handleRefreshDied = async () => {
        ...
        try {
            ...
            for (const record of currentDied) {
                ...
                if (res.status_msg.includes('存續中') || res.status_msg.includes('未開始') || res.status_msg.includes('存续中') || res.status_msg.includes('未开始')) {
                    errCount++;
                } else {
                    const cleanOutput = replaceUndefinedWithNull({
                        tradeId: record.tradeId, 
                    ...
                    delete cleanOutput.history_records; delete cleanOutput.plot_data;
                    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'sip_trade_dqaq_input_died', record.inputId), cleanInput);
                    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'sip_holding_dqaq_output_died', record.outputId), cleanOutput);
                }
            }
            await loadRecords();
    ...
    const handleSaveRecordEdit = async () => {
        ...
        try {
            ...
            alert("修改失敗 (請檢查 JSON 格式是否正確): \n" + e.message);
        }
    };

    const handleDeleteRecord = async (id: string) => {
        if (!confirm("確定要永久刪除這條記錄嗎？不可恢復。")) return;
        try {
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="border-b border-gray-200 pb-4 flex justify-between items-center">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">统一管理您的 DQ-AQ 存续与历史持仓，执行实时定价、状态流转与接货覆写。</p>
                </div>
                {/* 全局 HKD 切换按钮 */}
                <button 
            ...
            <div className="bg-white shadow rounded-lg p-6 border border-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Database size={20} className="text-blue-600"/>【DQ-AQ 持仓板塊 (存續中)】</h2>
                </div>
                <div className="overflow-x-auto border rounded-lg mb-4 shadow-sm">
                    <table className="min-w-full text-xs text-left divide-y divide-gray-200">
                        <thead className="bg-gray-50 text-gray-600 font-medium">
                            <tr>
                                <Th label="名稱" sortKey="name" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="當前狀態" sortKey="status" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="幣種" sortKey="currency" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                ...
                                <Th label="槓桿" sortKey="leverage" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                ...
                                <Th label="敲入價" sortKey="koInPrice" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="敲出價" sortKey="koOutPrice" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="下一觀察日" sortKey="nextObs" align="center" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="最後結算日" sortKey="maturity" align="center" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <th className="px-3 py-2 text-center">點位圖</th>
                                ...
                                <Th label="預期完成率" sortKey="expRate" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="當前市值(淨價)" sortKey="mktVal" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="已結算股数" sortKey="settled" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="已鎖定未付股数" sortKey="locked" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="全價" sortKey="fullPrice" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {finalLiving.map(item => (
                                <tr key={item.id} className="hover:bg-blue-50/50">
                                    ...
                                    <td className="px-3 py-2 text-right text-red-600 underline decoration-dotted cursor-help" title={`未來期望股數: ${item.future.toFixed(2)}`}>{fmtPct(item.koProb)}</td>
                                    <td className="px-3 py-2 text-right text-blue-600 underline decoration-dotted cursor-help" title={`期望總股數: ${item.expTotal.toFixed(2)}`}>{fmtPct(item.expRate)}</td>
                ...
                <button onClick={handleRefreshLiving} disabled={loadingLiving} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-md flex justify-center items-center gap-2">
                    {loadingLiving ? <Loader2 className="animate-spin" size={18} /> : <RefreshCw size={18} />} {loadingLiving ? '重新測算與流轉...' : '刷新當前持仓 (獲取實時現價並提取交收流水)'}
                </button>
            </div>
            ...
            <div className="bg-white shadow rounded-lg p-6 border border-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><TrendingUp size={20} className="text-red-600"/>【DQ-AQ 風控模塊】</h2>
                </div>
                <div className="overflow-x-auto border rounded-lg shadow-sm">
                    <table className="min-w-full text-xs text-left divide-y divide-gray-200">
                        <thead className="bg-red-50 text-red-800 font-medium">
                            <tr>
                                <Th label="標的代碼" sortKey="ticker" currentSort={riskSort} onSort={toggleRiskSort} currentFilter={riskFilters} onFilter={updateRiskFilter} />
                                <Th label="合約名稱" sortKey="name" currentSort={riskSort} onSort={toggleRiskSort} currentFilter={riskFilters} onFilter={updateRiskFilter} />
                                <Th label="暴露成本價(Strike)" sortKey="costPrice" align="right" currentSort={riskSort} onSort={toggleRiskSort} currentFilter={riskFilters} onFilter={updateRiskFilter} />
                                <Th label="暴露股數" sortKey="exposureShares" align="right" currentSort={riskSort} onSort={toggleRiskSort} currentFilter={riskFilters} onFilter={updateRiskFilter} />
                                ...
                                <Th label="暴露盈虧比" sortKey="pnlRatio" align="right" currentSort={riskSort} onSort={toggleRiskSort} currentFilter={riskFilters} onFilter={updateRiskFilter} />
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {finalRisk.map(r => (
            ...
            <div className="bg-white shadow rounded-lg p-6 border border-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Database size={20} className="text-orange-600"/>【DQ-AQ 持仓板塊 (歷史)】</h2>
                </div>
                <div className="overflow-x-auto border rounded-lg mb-4 shadow-sm">
                    <table className="min-w-full text-xs text-left divide-y divide-gray-200">
                        <thead className="bg-gray-50 text-gray-600 font-medium">
                            <tr>
                                <Th label="名稱" sortKey="name" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="歷史狀態" sortKey="status" align="center" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="幣種" sortKey="currency" align="center" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                ...
                                <Th label="槓桿" sortKey="leverage" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="敲入價" sortKey="koInPrice" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="敲出價" sortKey="koOutPrice" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="最終結算股數" sortKey="settled" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="最終完成率" sortKey="expRate" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="全價" sortKey="fullPrice" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {finalDied.map(item => (
                ...
                <button onClick={handleRefreshDied} disabled={loadingDied} className="w-full py-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-md flex justify-center items-center gap-2">
                    {loadingDied ? <Loader2 className="animate-spin" size={18} /> : <RefreshCw size={18} />} {loadingDied ? '重新計算校驗中...' : '刷新歷史持仓 (校驗狀態準確性)'}
                </button>
            </div>
            ...
            {showDeliveryModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-4xl flex flex-col h-[80vh]">
                        <h3 className="font-bold text-xl mb-2 flex items-center gap-2 text-orange-600"><AlertCircle /> 重要風控攔截：檢測到新交收單！</h3>
                        <p className="text-sm text-gray-600 mb-4 border-l-4 border-orange-400 pl-2">
                            本次刷新引起了底層狀態流轉，系統共提取出了 <strong>{pendingDeliveries.length}</strong> 條全新的交收記錄。<br/>
                            是否立即將這些流水依據 TradeID <strong>覆寫同步</strong> 至實盤接貨庫 (get-stock) 中？
                        </p>
                        <div className="overflow-auto flex-1 border rounded-lg mb-4 bg-gray-50">
                            <table className="w-full text-xs text-left">
                                <thead className="bg-gray-100"><tr><th className="p-2">TradeID</th><th className="p-2">結算日</th><th className="p-2">標的</th><th className="p-2">方向</th><th className="p-2 text-right">股數</th><th className="p-2 text-right">結算金額</th></tr></thead>
                                <tbody>
                                    {pendingDeliveries.map((r, i) => (
                                        <tr key={i} className="border-b">
                                            <td className="p-2 font-mono text-gray-400">{r.tradeId.substring(0,6)}</td><td className="p-2">{r.date}</td>
                                            <td className="p-2 font-bold">{r.stockCode}</td><td className="p-2">{r.direction}</td>
                                            <td className={`p-2 text-right font-mono ${r.quantity<0?'text-red-600':''}`}>{r.quantity}</td>
                                            <td className="p-2 text-right font-mono">{r.amountNoFee}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button onClick={()=>setShowDeliveryModal(false)} className="px-6 py-2 bg-gray-200 text-gray-700 rounded font-bold">暫不同步 (稍後處理)</button>
                            <button onClick={handleConfirmDeliveries} disabled={syncingDeliveries} className="px-6 py-2 bg-orange-600 text-white rounded font-bold flex items-center gap-2">
                                {syncingDeliveries ? <Loader2 className="animate-spin" size={16}/> : <Save size={16}/>} 確認精準覆寫至 Get-Stock 庫
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {chartData && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col relative overflow-hidden">
                                    ...
                                    <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                                        <div className="flex justify-between items-end mb-4">
                                            ...
                                            <div className="text-right">
                                                <span className="text-xs text-gray-500 mr-1">現價</span>
                                            ...
                                            <div className="flex-1 relative h-16 mx-4 select-none">
                                                ...
                                                <div className="absolute top-1/2" style={{ left: `${getPos(strike)}%` }}>
                                                    ...
                                                    <div className="absolute transform -translate-x-1/2 translate-y-3 flex flex-col items-center w-max">
                                                        ...
                                                        <span className="text-[9px] text-gray-400">{strike.toFixed(4)}</span>
                                                    </div>
                                                </div>
                                                <div className="absolute top-1/2" style={{ left: `${getPos(ko)}%` }}>
                                                    ...
                                                    <div className="absolute transform -translate-x-1/2 translate-y-3 flex flex-col items-center w-max">
                                                        ...
                                                        <span className="text-[9px] text-gray-400">{ko.toFixed(4)}</span>
                                                    </div>
                                                </div>
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-5xl flex flex-col h-[85vh] max-h-[90vh]">
                        ...
                        <p className="text-xs text-gray-500 mb-2 border-l-2 border-orange-400 pl-2">
                            警告：直接修改 Raw JSON 屬於高階操作，請確保 JSON 格式合法且結構正確。
                        </p>
                        ...
                        <div className="flex justify-end gap-3 pt-2 border-t">
                            ...
                            <button onClick={handleSaveRecordEdit} className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-md text-sm font-bold flex items-center gap-2 transition-colors"><Save size={16}/> 保存強制覆蓋</button>
                        </div>
                    </div>
                </div>
            )}
