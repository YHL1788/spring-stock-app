...
export function buildInitialPortfolioState(params: {
  ...
  const fxRates: Record<string, number> = { HKD: 1, ...(params.fallbackFxRates || {}) };
  const details: InitialPortfolioDetail[] = [];

  START_ASSETS.forEach((asset) => {
    const configFx = params.configs[asset.id]?.baseFxRates || {};
    ...
    rows.forEach((record, index) => {
      ...
      const fxRate = toNumber(configFx[currency] ?? fxRates[currency] ?? 1, 1);
...
export function calculateNavSeries(initialState: InitialPortfolioState | null, snapshots: PortfolioSnapshot[], flows: CapitalFlow[]): NavPoint[] {
  ...
  sortedSnapshots.forEach((snapshot) => {
    const netFlow = sumFlowsBetween(sortedFlows, previousDate, snapshot.snapshotDate);
    const profit = snapshot.totalMarketValueHKD - previousAsset - netFlow;
    const denominator = Math.abs(previousAsset) > 0.000001 ? previousAsset : 1;
    const periodReturn = profit / denominator;
    previousNav = previousNav * (1 + periodReturn);
...
function sumFlowsBetween(flows: CapitalFlow[], fromDateExclusive: string, toDateInclusive: string) {
  return flows.reduce((sum, flow) => {
    if (flow.flowDate <= fromDateExclusive || flow.flowDate > toDateInclusive) return sum;
    const amount = Math.abs(toNumber(flow.amountHKD, 0));
    return sum + (flow.direction === 'OUT' ? -amount : amount);
  }, 0);
}
