...
const Th = ({ label, sortKey, filterKey, currentSort, onSort, currentFilter, onFilter, align='left', className='' }: any) => {
    ...
    return (
        <th className={`px-3 py-2 whitespace-nowrap align-top group ${textClass} ${className}`}>
...
export default function DQAQHoldingPage() {
    ...
    // 点位图 Modal
    const [chartData, setChartData] = useState<{name: string, ticker: string, currency: string, current: number, strike: number, ko: number, recordId: string} | null>(null);
    const [plotData, setPlotData] = useState<PlotData | null>(null);
    const [isPlotLoading, setIsPlotLoading] = useState(false);
    const [plotTradeFlows, setPlotTradeFlows] = useState<DQAQTradeFlow[]>([]);
    const [isTradeFlowLoading, setIsTradeFlowLoading] = useState(false);
    // 后台库
    ...
    const handleGeneratePlot = async (tradeId: string) => {
        ...
        setIsTradeFlowLoading(true);
        try {
            const rec = livingRecords.find(r => r.tradeId === tradeId) || diedRecords.find(r => r.tradeId === tradeId);
            if (!rec) throw new Error("找不到对应的持仓记录");

            const tradeFlowQuery = query(
                collection(db, 'artifacts', APP_ID, 'public', 'data', 'sip_holding_dqaq_output_get-stock'),
                where('tradeId', '==', tradeId)
            );

            // 图表路径与该合约产生的接货流水使用同一个 tradeId 并行获取。
            const [{ res }, tradeFlowSnapshot] = await Promise.all([
                evaluateDQAQ(rec),
                getDocs(tradeFlowQuery)
            ]);
            ...
            const relatedFlows = tradeFlowSnapshot.docs
                .map(flowDoc => ({ id: flowDoc.id, ...flowDoc.data() } as DQAQTradeFlow))
                .sort((a, b) => {
                    const aTime = getTime(a.date) || getTime(a.createdAt);
                    const bTime = getTime(b.date) || getTime(b.createdAt);
                    return bTime - aTime;
                });
            setPlotTradeFlows(relatedFlows);
        } catch (e: any) {
            alert("生成模拟点位图失败: " + e.message);
        } finally {
            setIsPlotLoading(false);
            setIsTradeFlowLoading(false);
        }
    };
    ...
    const processLiving = useMemo(() => livingRecords.map((r: MergedRecord) => {
        ...
        const name = `${basic.broker} | ${underlying.stock_name||underlying.ticker}`;
        ...
        return {
            ...
            account: basic.account,
            name, tradeDate: basic.trade_date || '', status: res.status_msg, currency: basic.currency,
            ...
            initialPrice: Number(underlying.spot_price) || 0,
    ...
    const processDied = useMemo(() => diedRecords.map((r: MergedRecord) => {
        ...
        const koPrice = underlying.spot_price * basic.ko_barrier_pct;
        return {
            id: r.tradeId, name: `${basic.broker} | ${underlying.stock_name||underlying.ticker}`,
            tradeDate: basic.trade_date || '', status: res.status_msg, currency: basic.currency, dir: basic.contract_type, leverage: basic.leverage,
            ticker: underlying.ticker,
            S0: Number(underlying.spot_price) || 0,
            initialPrice: Number(underlying.spot_price) || 0,
            currentPrice: Number(underlying.current_price || underlying.spot_price) || 0,
            strikeRaw: basic.strike_pct,
            koOutRaw: basic.ko_barrier_pct,
            koInPrice: strikePrice.toFixed(4), koOutPrice: koPrice.toFixed(4),
            koInPct: fmtPct(basic.strike_pct), koOutPct: fmtPct(basic.ko_barrier_pct),
    ...
    return (
        <div className="space-y-8 pb-10">
                ...
                <div className="overflow-x-auto overflow-y-auto max-h-[500px] border rounded-lg mb-4 shadow-sm relative scrollbar-thin">
                    <table className="min-w-full text-xs text-left divide-y divide-gray-200">
                        <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0 z-20 shadow-sm [&>tr>th]:bg-gray-50">
                            <tr>
                                <Th label="当前状态" sortKey="status" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} align="center" className="sticky left-0 top-0 z-40 w-[150px] min-w-[150px] bg-gray-50 shadow-[1px_0_0_rgba(0,0,0,0.08)]" />
                                <Th label="交易日期" sortKey="tradeDate" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} align="center" className="sticky left-[150px] top-0 z-40 w-[105px] min-w-[105px] bg-gray-50 shadow-[1px_0_0_rgba(0,0,0,0.08)]" />
                                <Th label="名称" sortKey="name" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} className="sticky left-[255px] top-0 z-40 w-[220px] min-w-[220px] bg-gray-50 shadow-[2px_0_0_rgba(0,0,0,0.1)]" />
                                <Th label="币种" sortKey="currency" align="center" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                ...
                                <Th label="初始价" sortKey="initialPrice" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                                <Th label="现价" sortKey="currentPrice" align="right" currentSort={livingSort} onSort={toggleLivingSort} currentFilter={livingFilters} onFilter={updateLivingFilter} />
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {finalLiving.map(item => (
                                <tr
                                    key={item.id}
                                    className={
                                        item.dir === 'DQ' && item.currentPrice > Number(item.koInPrice)
                                            ? 'group bg-yellow-50 hover:bg-yellow-100'
                                            : item.dir === 'AQ' && item.currentPrice < Number(item.koInPrice)
                                                ? 'group bg-red-50 hover:bg-red-100'
                                                : 'group hover:bg-blue-50/50'
                                    }
                                >
                                    <td className={`sticky left-0 z-10 w-[150px] min-w-[150px] px-3 py-2 text-center font-bold text-green-700 shadow-[1px_0_0_rgba(0,0,0,0.05)] ${
                                        item.dir === 'DQ' && item.currentPrice > Number(item.koInPrice)
                                    ...
                                    <td className={`sticky left-[150px] z-10 w-[105px] min-w-[105px] px-3 py-2 text-center font-mono shadow-[1px_0_0_rgba(0,0,0,0.05)] ${
                                        item.dir === 'DQ' && item.currentPrice > Number(item.koInPrice)
                                            ? 'bg-yellow-50 group-hover:bg-yellow-100'
                                            : item.dir === 'AQ' && item.currentPrice < Number(item.koInPrice)
                                                ? 'bg-red-50 group-hover:bg-red-100'
                                    ...
                                    <td className={`sticky left-[255px] z-10 w-[220px] min-w-[220px] px-3 py-2 font-medium whitespace-nowrap shadow-[2px_0_0_rgba(0,0,0,0.08)] ${
                                        item.dir === 'DQ' && item.currentPrice > Number(item.koInPrice)
                                            ? 'bg-yellow-50 group-hover:bg-yellow-100'
                                            : item.dir === 'AQ' && item.currentPrice < Number(item.koInPrice)
                                                ? 'bg-red-50 group-hover:bg-red-100'
                                    ...
                                    <td className="px-3 py-2 text-right font-mono">{item.initialPrice.toFixed(4)}</td>
                                    <td className="px-3 py-2 text-right font-mono text-blue-700">{item.currentPrice.toFixed(4)}</td>
                ...
                <div className="mt-4 flex items-center justify-between bg-white px-4 py-3 rounded border border-red-100 shadow-sm">
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        <span className="text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded border border-red-100">
                            {isHKDView ? '※手动入库已在折算视图下暂停，保护原始暴露数据' : '*后台暗线逻辑：已按标的(Ticker)合并计算总暴露成本与股数'}
                        </span>
                    </div>
            ...
            <div className="bg-white shadow rounded-lg p-6 border border-gray-200">
                ...
                <div className="overflow-x-auto overflow-y-auto max-h-[500px] border rounded-lg mb-4 shadow-sm relative scrollbar-thin">
                    <table className="min-w-full text-xs text-left divide-y divide-gray-200">
                        <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0 z-20 shadow-sm [&>tr>th]:bg-gray-50">
                            <tr>
                                <Th label="当前状态" sortKey="status" align="center" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} className="sticky left-0 top-0 z-40 w-[150px] min-w-[150px] bg-gray-50 shadow-[1px_0_0_rgba(0,0,0,0.08)]" />
                                <Th label="交易日期" sortKey="tradeDate" align="center" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} className="sticky left-[150px] top-0 z-40 w-[105px] min-w-[105px] bg-gray-50 shadow-[1px_0_0_rgba(0,0,0,0.08)]" />
                                <Th label="名称" sortKey="name" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} className="sticky left-[255px] top-0 z-40 w-[220px] min-w-[220px] bg-gray-50 shadow-[2px_0_0_rgba(0,0,0,0.1)]" />
                                ...
                                <Th label="初始价" sortKey="initialPrice" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                                <Th label="现价" sortKey="currentPrice" align="right" currentSort={diedSort} onSort={toggleDiedSort} currentFilter={diedFilters} onFilter={updateDiedFilter} />
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {finalDied.map(item => (
                                <tr key={item.id} className="group hover:bg-orange-50/30">
                                    <td className="sticky left-0 z-10 w-[150px] min-w-[150px] bg-white group-hover:bg-orange-50 px-3 py-2 text-orange-700 font-bold text-center shadow-[1px_0_0_rgba(0,0,0,0.05)]">{item.status}</td>
                                    <td className="sticky left-[150px] z-10 w-[105px] min-w-[105px] bg-white group-hover:bg-orange-50 px-3 py-2 text-center font-mono shadow-[1px_0_0_rgba(0,0,0,0.05)]">{item.tradeDate}</td>
                                    <td className="sticky left-[255px] z-10 w-[220px] min-w-[220px] bg-white group-hover:bg-orange-50 px-3 py-2 font-medium whitespace-nowrap shadow-[2px_0_0_rgba(0,0,0,0.08)]">{item.name}</td>
                                    <td className="px-3 py-2 text-center">{item.currency}</td>
                                    <td className={`px-3 py-2 text-center font-bold ${item.dir==='DQ'?'text-red-600':'text-green-600'}`}>{item.dir}</td>
                                    <td className="px-3 py-2 text-right">{item.leverage}x</td>
                                    <td className="px-3 py-2 text-right font-mono">{item.initialPrice.toFixed(4)}</td>
                                    ...
                                    <td className="px-3 py-2 text-center">
                                        <button
                                            ...
                                            className="text-orange-500 hover:bg-orange-100 p-1 rounded transition-colors"
                                            title="查看点位图及关联交易流水"
                                        >
                                            <LineChart size={16}/>
                                        </button>
            ...
            {chartData && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-5xl max-h-[92vh] overflow-y-auto flex flex-col relative animate-in fade-in duration-200">
                        <div className="flex justify-between items-center mb-4 border-b pb-4">
                            <div>
                            ...
                            <button onClick={() => { setChartData(null); setPlotData(null); setPlotTradeFlows([]); }} className="text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 p-2 rounded-full transition-all self-start">
                                <X size={20}/>
                            </button>
                        </div>
                        ...
                        {(isTradeFlowLoading || plotTradeFlows.length > 0) && (
                            <div className="mt-5 rounded-lg border border-orange-200 overflow-hidden">
                                <div className="px-4 py-3 bg-orange-50 border-b border-orange-200 flex items-center justify-between">
                                    <h4 className="font-bold text-sm text-orange-900">关联交易流水</h4>
                                    {!isTradeFlowLoading && (
                                        <span className="text-xs text-orange-700">共 {plotTradeFlows.length} 笔</span>
                                    )}
                                </div>
                                {isTradeFlowLoading ? (
                                        ...
                                        <table className="min-w-full text-xs whitespace-nowrap">
                                            <thead className="sticky top-0 bg-gray-50 text-gray-600">
                                                <tr>
                                                    ...
                                                    <th className="px-3 py-2 text-left">股票代码 / 名称</th>
                                                    <th className="px-3 py-2 text-right">成交股数</th>
                                                    <th className="px-3 py-2 text-right">成交价</th>
                                                    <th className="px-3 py-2 text-right">交易金额</th>
                                                    <th className="px-3 py-2 text-right">金额 HKD</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-100">
                                                {plotTradeFlows.map(flow => (
                                                    <tr key={flow.id} className="hover:bg-orange-50/40">
                                                        <td className="px-3 py-2 font-mono">{flow.date || formatTime(flow.createdAt) || '--'}</td>
                                                        <td className="px-3 py-2">{flow.account || '--'}</td>
                                                        <td className={`px-3 py-2 text-center font-bold ${(flow.direction || '').toUpperCase() === 'SELL' ? 'text-red-600' : 'text-green-600'}`}>
                                                            {(flow.direction || '--').toUpperCase()}
                                                        </td>
                                                        <td className="px-3 py-2">
                                                            <span className="font-mono font-bold">{flow.stockCode || chartData.ticker}</span>
                                                            {flow.stockName && <span className="ml-2 text-gray-500">{flow.stockName}</span>}
                                                        </td>
                                                        <td className="px-3 py-2 text-right font-mono">{Number(flow.quantity || 0).toLocaleString('en-US', { maximumFractionDigits: 4 })}</td>
                                                        <td className="px-3 py-2 text-right font-mono">{Number(flow.priceNoFee || 0).toFixed(4)}</td>
                                                        <td className="px-3 py-2 text-right font-mono">{Number(flow.amountNoFee || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {chartData.currency}</td>
                                                        <td className="px-3 py-2 text-right font-mono font-bold">{Number(flow.hkdAmount || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                        ...
                        <div className="flex justify-between items-center mt-4 pt-4 border-t text-xs text-gray-500">
                            ...
                            <button onClick={() => { setChartData(null); setPlotData(null); setPlotTradeFlows([]); }} className="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-md transition-colors">
                                关闭图表
                            </button>
                        </div>
                    </div>
